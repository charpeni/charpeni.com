...
> [!NOTE]
> As of [pnpm 11](https://pnpm.io/blog/releases/11.0), `minimumReleaseAge` defaults to `1440` (1 day), so you get this protection out of the box. The sections below still apply if you want to tune the value, configure exclusions, or opt out.

If you're using pnpm 10.16+, you can configure it directly in your `pnpm-workspace.yaml`:

> [!TIP]
> This can also be set globally to apply to every project on your machine. The global `rc` file lives at `~/Library/Preferences/pnpm/rc` on macOS, `~/.config/pnpm/rc` on Linux, and `~/AppData/Local/pnpm/config/rc` on Windows (you can also run `pnpm config get globalconfig` to print the exact path on your system). Note that it uses INI syntax with a kebab-case key:
>
> ```ini
...
> [!TIP]
> This can also be set globally in `~/.yarnrc.yml` to apply to every project on your machine.
...
> [!TIP]
> This can also be set globally in `~/.npmrc` to apply to every project on your machine.
...
> [!TIP]
> This can also be set globally in `~/.bunfig.toml` to apply to every project on your machine.
