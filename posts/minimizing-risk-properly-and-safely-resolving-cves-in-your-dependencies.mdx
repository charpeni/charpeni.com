...
> [!TIP] > `-R,--recursive` flag will also report transitive dependencies. **Required** when investigating an issue that's not a direct dependency.
...
> [!TIP] > `-R,--recursive` flag may or may not be required, depending on whether the dependency you are updating is a direct or transitive dependency.
...
> [!NOTE] > `-R,--recursive` flag is omitted here as `gulp` is a direct dependency.
