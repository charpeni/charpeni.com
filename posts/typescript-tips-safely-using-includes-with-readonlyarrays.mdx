...
```typescript twoslash
// @errors: 2345
type AllFruits = 'Apple' | 'Banana' | 'Orange' | 'Grape' | 'Watermelon' | 'Pineapple' | 'Mango' | 'Strawberry' | 'Peach' | 'Pear';
const TOP_PICKS = ['Apple', 'Peach'] as const satisfies ReadonlyArray<AllFruits>;
// ---cut---
function scenario(selection: AllFruits) {
  if (TOP_PICKS.includes(selection)) {
    // Do something.
  }
}
```
...
const TOP_PICKS = [
  'Apple',
  'Peach',
] as const satisfies ReadonlyArray<AllFruits>;
// ---cut---
function scenario(selection: AllFruits) {
