...
Using [`Array.protoype.reduce`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/Reduce) was probably the proper way to achieve a grouping, even though thoughts on reducers are often mitigated as the syntax tends to make it harder to read.
...
> ⚠️ This example goes against immutability as we're mutating the object, but it was worth the example as a fair usage of a group by.
