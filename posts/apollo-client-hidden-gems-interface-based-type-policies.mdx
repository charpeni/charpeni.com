...
So, instead of defining the same field policy for each `DataPoint`, I extracted the common field to an interface and defined the field policy on the interface instead:
