...
Run the app or the tests, is it working or not? Let git know and move on automatically to the next step. Then, we repeat the last steps until we find the commit that introduced the bug by narrowing them down.
